...
function enhanceNavigation(){document.body.classList.add('page-ready');let path=location.pathname.replace(/index.html$/,'');document.querySelectorAll('[data-nav]').forEach(a=>{let href=new URL(a.getAttribute('href'),location.href).pathname.replace(/index.html$/,'');if(href===path)a.classList.add('active');a.addEventListener('click',e=>{if(e.ctrlKey||e.metaKey)return;e.preventDefault();fadeTo(a.href)})});document.querySelectorAll('.card,.stat,.table').forEach((el,i)=>{el.style.animationDelay=(Math.min(i,8)*35)+'ms';el.classList.add('fade-item')})}
